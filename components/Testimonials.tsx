...
const TESTIMONIALS = [
  ...
  { name: 'David K.', role: 'E-commerce Brand', text: "We went from manual support to fully autonomous. Sales up 200% since the upgrade.", city: 'London' },
  { name: 'Jessica W.', role: 'Legal Consultant', text: "The website speed is unmatched. My clients are impressed before they even speak to me.", city: 'Chicago' },
  { name: 'Ryan B.', role: 'SaaS Founder', text: "Technical expertise is next level. They built a custom agent that knows our API docs perfectly.", city: 'Austin' },
  { name: 'Omar S.', role: 'Property Manager', text: "I used to spend 4 hours a day on calls. Now OPRAX handles it. Freedom.", city: 'Istanbul' },
];
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 h-[600px] md:h-[800px] overflow-hidden relative">
          ...
          {[
            ...
            <div key={idx} className={`flex flex-col gap-6 md:gap-8 ${col.speed} ${idx > 0 ? 'hidden md:flex' : 'flex'}`}>
              {col.items.map((t, i) => (
                <div key={i} className="glass-premium p-6 md:p-8 rounded-[2rem] neon-border bg-[#0A0E27]/40 group shrink-0 flex flex-col justify-between h-auto">
                  
                  {/* Top: User Profile */}
                  <div className="flex items-center gap-4 mb-5">
                    <div className="w-10 h-10 md:w-12 md:h-12 bg-gradient-to-br from-[#00D4FF] to-[#8B5CF6] rounded-full flex items-center justify-center font-black text-white text-base md:text-lg shadow-lg shrink-0">
                      {t.name[0]}
                    </div>
                    <div>
                      <div className="font-black text-white text-sm md:text-base">{t.name}</div>
                      <div className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">{t.role}</div>
                    </div>
                  </div>

                  {/* Middle: Content */}
                  <p className="text-gray-300 font-medium leading-relaxed mb-6 text-xs md:text-sm">"{t.text}"</p>
                  
                  {/* Bottom: Stars */}
                  <div className="flex text-[#bef264] text-xs space-x-0.5">
                    {'★'.repeat(5)}
                  </div>
